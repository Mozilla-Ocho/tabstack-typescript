...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { TABSTACK_API_KEY: requireValue(readEnv('TABSTACK_API_KEY') ?? client.apiKey, 'set TABSTACK_API_KEY environment variable or provide apiKey client option'), TABSTACK_BASE_URL: readEnv('TABSTACK_BASE_URL') ?? client.baseURL ?? undefined };
