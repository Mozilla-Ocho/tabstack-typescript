...
const prompt = `Runs JavaScript code to interact with the Tabstack API.
...
export function codeTool(): McpTool {
  ...
  const handler = async (client: Tabstack, args: any): Promise<ToolCallResult> => {
    ...
    const res = await fetch(codeModeEndpoint, {
      ...
      headers: {
        ...
        client_envs: JSON.stringify({
          TABSTACK_API_KEY: requireValue(
            readEnv('TABSTACK_API_KEY') ?? client.apiKey,
            'set TABSTACK_API_KEY environment variable or provide apiKey client option',
          ),
          TABSTACK_BASE_URL: readEnv('TABSTACK_BASE_URL') ?? client.baseURL ?? undefined,
        }),
      },
      body: JSON.stringify({
