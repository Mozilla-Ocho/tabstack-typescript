...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    ...
    perLanguage: {
      ...
      php: {
        method: 'agent->automate',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(apiKey: 'My API Key');\n\n$automateEvent = $client->agent->automateStream(\n  task: 'Find the top 3 trending repositories and extract their names, descriptions, and star counts',\n  data: (object) [],\n  geoTarget: ['country' => 'US'],\n  guardrails: 'browse and extract only, don\\'t interact with repositories',\n  interactive: false,\n  maxIterations: 50,\n  maxValidationAttempts: 3,\n  trustedHostnames: ['string'],\n  unsafeMode: false,\n  url: 'https://github.com/trending',\n);\n\nvar_dump($automateEvent);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      php: {
        method: 'agent->automateInput',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(apiKey: 'My API Key');\n\n$response = $client->agent->automateInput(\n  'requestID',\n  cancelled: true,\n  fields: [['ref' => 'E42', 'value' => 'user@example.com']],\n);\n\nvar_dump($response);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      php: {
        method: 'agent->research',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(apiKey: 'My API Key');\n\n$researchEvent = $client->agent->researchStream(\n  query: 'What are the latest developments in quantum computing?',\n  fetchTimeout: 30,\n  mode: 'fast',\n  nocache: false,\n);\n\nvar_dump($researchEvent);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      php: {
        method: 'extract->json',
        example:
          "<?php\n\nrequire_once dirname(__DIR__) . '/vendor/autoload.php';\n\n$client = new Client(apiKey: 'My API Key');\n\n$response = $client->extract->json(\n  jsonSchema: [\n    'properties' => [\n      'stories' => [\n        'items' => [\n          'properties' => [\n            'author' => [\n              'description' => 'Author username', 'type' => 'string'\n            ],\n            'points' => ['description' => 'Story points', 'type' => 'number'],\n            'title' => ['description' => 'Story title', 'type' => 'string'],\n          ],\n          'type' => 'object',\n        ],\n        'type' => 'array',\n      ],\n    ],\n    'type' => 'object',\n  ],\n  url: 'https://news.ycombinator.com',\n  effort: 'standard',\n  geoTarget: ['country' => 'US'],\n  nocache: false,\n);\n\nvar_dump($response);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    response:
      '{ content: string; url: string; metadata?: { author?: string; created_at?: string; creator?: string; description?: string; favicon?: string; image?: string; keywords?: string[]; modified_at?: string; page_count?: number; pdf_version?: string; producer?: string; publisher?: string; site_name?: string; subject?: string; title?: string; type?: string; url?: string; }; }',
    markdown:
      "## markdown\n\n`client.extract.markdown(url: string, content?: 'main' | 'full', effort?: 'min' | 'standard' | 'max', geo_target?: { country?: string; }, metadata?: boolean, nocache?: boolean): { content: string; url: string; metadata?: object; }`\n\n**post** `/extract/markdown`\n\nFetches a URL and converts its HTML content to clean Markdown format with optional metadata extraction\n\n### Parameters\n\n- `url: string`\n  URL to fetch and convert to markdown\n\n- `content?: 'main' | 'full'`\n  Content scope. \"main\" (default) returns the main article content; \"full\" returns the whole page, including navigation, footer, and links.\n\n- `effort?: 'min' | 'standard' | 'max'`\n  Fetch effort level controlling speed vs. capability tradeoff. \"min\": fastest, no fallback (1-5s). \"standard\": balanced with enhanced reliability (default, 3-15s). \"max\": full browser rendering for JS-heavy sites (15-60s).\n\n- `geo_target?: { country?: string; }`\n  Optional geotargeting parameters for proxy requests\n  - `country?: string`\n    Country code using ISO 3166-1 alpha-2 standard (2 letters, e.g., \"US\", \"GB\", \"JP\").\nSee: https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2\n\n- `metadata?: boolean`\n  Include extracted metadata (Open Graph and HTML metadata) as a separate field in the response\n\n- `nocache?: boolean`\n  Bypass cache and force fresh data retrieval\n\n### Returns\n\n- `{ content: string; url: string; metadata?: { author?: string; created_at?: string; creator?: string; description?: string; favicon?: string; image?: string; keywords?: string[]; modified_at?: string; page_count?: number; pdf_version?: string; producer?: string; publisher?: string; site_name?: string; subject?: string; title?: string; type?: string; url?: string; }; }`\n\n  - `content: string`\n  - `url: string`\n  - `metadata?: { author?: string; created_at?: string; creator?: string; description?: string; favicon?: string; image?: string; keywords?: string[]; modified_at?: string; page_count?: number; pdf_version?: string; producer?: string; publisher?: string; site_name?: string; subject?: string; title?: string; type?: string; url?: string; }`\n\n### Example\n\n```typescript\nimport Tabstack from '@tabstack/sdk';\n\nconst client = new Tabstack();\n\nconst response = await client.extract.markdown({ url: 'https://example.com/blog/article' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
...
const EMBEDDED_READMES: { language: string; content: string }[] = [
  ...
  },
  {
    language: 'php',
    content:
      '# Tabstack PHP API Library\n\nThe Tabstack PHP library provides convenient access to the Tabstack REST API from any PHP 8.1.0+ application.\n\n## Installation\n\nTo use this package, install via Composer by adding the following to your application\'s `composer.json`:\n\n```json\n{\n  "repositories": [\n    {\n      "type": "vcs",\n      "url": "git@github.com:stainless-sdks/tabstack-php.git"\n    }\n  ],\n  "require": {\n    "org-placeholder/tabstack": "dev-main"\n  }\n}\n```\n\n## Usage\n\n```php\n<?php\n\n$client = new Client(apiKey: getenv(\'TABSTACK_API_KEY\') ?: \'My API Key\');\n\n$automateEvent = $client->agent->automateStream(\n  task: \'Find the top 3 trending repositories and extract their names, descriptions, and star counts\',\n);\n\nvar_dump($automateEvent);\n```',
  },
  {
    language: 'python',
    content:
