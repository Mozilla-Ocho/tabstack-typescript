...
import { Agent, AgentAutomateInputParams, AgentAutomateInputResponse, AgentAutomateParams, AgentResearchParams, AutomateEvent, ResearchEvent } from './resources/agent';
import { Extract, ExtractJsonParams, ExtractJsonResponse, ExtractMarkdownParams, ExtractMarkdownResponse } from './resources/extract';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['TABSTACK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TABSTACK_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Tabstack API. 
 */
export class Tabstack {
   ...
   * API Client for interfacing with the Tabstack API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['TABSTACK_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['TABSTACK_BASE_URL'] ?? https://api.tabstack.ai/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TABSTACK_BASE_URL'),
    apiKey = readEnv('TABSTACK_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TabstackError(
        'The TABSTACK_API_KEY environment variable is missing or empty; either provide it, or instantiate the Tabstack client with an apiKey option, like new Tabstack({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.tabstack.ai/v1`,
    };
    ...
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('TABSTACK_LOG'), 'process.env[\'TABSTACK_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.tabstack.ai/v1';
  }
  ...
  static Tabstack = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static TabstackError = Errors.TabstackError;
...
export declare namespace Tabstack {
